  ...
  object PersistentReprSerializer extends Serializer[PersistentRepr] {
    ...
    def deserialize(implicit format: Formats) = {
      case (TypeInfo(Clazz, _), json) =>
        ...
        val eventClass = Class.forName(x.payloadType)
        implicit val manifest = Manifest.classType(eventClass)
        PersistentRepr(Extraction.extract(x.payload), x.sequenceNr, x.persistenceId, x.deleted, sender = x.sender)
    }
    def serialize(implicit format: Formats) = {
      case x: PersistentRepr =>
        val mapping = Mapping(decompose(x.payload), x.payload.getClass.getName, x.sequenceNr, x.persistenceId, x.deleted, x.sender)
        decompose(mapping)
    }

    case class Mapping(payload: JValue, payloadType: String, sequenceNr: Long, persistenceId: String, deleted: Boolean, sender: ActorRef)
  }
}
